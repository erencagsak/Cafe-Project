...
{
    public partial class manager_Tables_Table2 : DevExpress.XtraEditors.XtraForm
    {
        BaglantiSinif bgl = new BaglantiSinif();

        // SQL bağlantılarını açar.
        public manager_Tables_Table2()
        {
            InitializeComponent();
        }
        private void manager_Tables_Table2_Load(object sender, EventArgs e)
        {
            screenSettings();
            // Ekranı ayarla.
        ...
        private void btn_Add_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (dataGriedView_Menu.SelectedRows.Count > 0)
                {
                    ...
                    using (SqlCommand ekle = new SqlCommand(@"insert into Table2 (name, number, price) values (@name, @number, @price)", baglanti))
                    {
        ...
        private void btn_Delete_Click(object sender, EventArgs e)
        {
            ...
            string sqlTable = "SELECT * FROM Table2 WHERE id=@id";
            ...
            try
            {
                if (readerTable.Read())
                {
                    ...
                    if (DialogResult.Yes == situation)
                    {
                        ...
                        string tableDeleteSQL = "DELETE FROM Table2 WHERE id=@id";
            ...
            try
            {
                ...
                if (DialogResult.Yes == situation)
                {
                    if (dataGriedView_Order.Rows.Count > 0)
                    {
                        ...
                        SqlCommand cmdTotalPrice = new SqlCommand("INSERT INTO day_TotalPrice_Tables (table_Name,table_Price) VALUES ('Masa 2','" + txtBox_Paid.Text + "')", baglanti);

                        cmdTotalPrice.ExecuteNonQuery();

                        string truncateTable = "TRUNCATE TABLE Table2";
        ...
        public void change_Appearance_data_GriedView_Order()
        {
            ...
            data_GriedView_Order_Timer.Enabled = false;
            data_GriedView_Order_Timer.Stop();

            SqlDataAdapter adapterOrder = new SqlDataAdapter("SELECT id,name,number,price FROM Table2", baglanti);
            ...
            data_GriedView_Order_Timer.Enabled = true;
            data_GriedView_Order_Timer.Start();
        }
        public void change_Appearance_data_GriedView_Menu()
        {
            dataGriedView_Menu.ColumnHeadersDefaultCellStyle.BackColor = Color.Moccasin;
        ...
        public void refresh_DataGriedView_Order()
        {
            ...
            string selectOrder = "SELECT id,name,number,price FROM Table2";
            SqlDataAdapter tableAdapter = new SqlDataAdapter(selectOrder, baglanti);
        ...
        public void call_Price_For_txtbox_Paid()
        {
            ...
            try
            {
                SqlCommand cmdTopla = new SqlCommand("SELECT price FROM Table2", baglanti);
