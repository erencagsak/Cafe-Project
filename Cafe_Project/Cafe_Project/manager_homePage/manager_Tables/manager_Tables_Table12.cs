...
{
    public partial class manager_Tables_Table12 : DevExpress.XtraEditors.XtraForm
    {
        BaglantiSinif bgl = new BaglantiSinif();

        // SQL bağlantılarını açar.
        public manager_Tables_Table12()
        {
            InitializeComponent();
        }
        ...
        private void btn_Add_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (dataGriedView_Menu.SelectedRows.Count > 0)
                {
                    ...
                    using (SqlCommand ekle = new SqlCommand(@"insert into Table12 (name, number, price) values (@name, @number, @price)", baglanti))
                    {
        ...
        private void btn_Delete_Click(object sender, EventArgs e)
        {
            ...
            string sqlTable = "SELECT * FROM Table12 WHERE id=@id";
            ...
            try
            {
                if (readerTable.Read())
                {
                    ...
                    if (DialogResult.Yes == situation)
                    {
                        ...
                        string tableDeleteSQL = "DELETE FROM Table12 WHERE id=@id";
            ...
            try
            {
                ...
                if (DialogResult.Yes == situation)
                {
                    if (dataGriedView_Order.Rows.Count > 0)
                    {
                        ...
                        SqlCommand cmdTotalPrice = new SqlCommand("INSERT INTO day_TotalPrice_Tables (table_Name,table_Price) VALUES ('Masa 12','" + txtBox_Paid.Text + "')", baglanti);

                        cmdTotalPrice.ExecuteNonQuery();

                        string truncateTable = "TRUNCATE TABLE Table12";
        ...
        private void manager_Tables_Table12_Load(object sender, EventArgs e)
        {
            screenSettings();
            // Ekranı ayarla.
        ...
        public void change_Appearance_data_GriedView_Order()
        {
            ...
            SqlDataAdapter adapterOrder = new SqlDataAdapter("SELECT id,name,number,price FROM Table12", baglanti);
            DataTable tableOrder = new DataTable();
            adapterOrder.Fill(tableOrder);

            tableOrder.Columns["id"].ColumnName = "ID";
            tableOrder.Columns["name"].ColumnName = "name";
            tableOrder.Columns["number"].ColumnName = "number";
            tableOrder.Columns["price"].ColumnName = "price";
        ...
        public void refresh_DataGriedView_Order()
        {
            ...
            string selectSQL = "SELECT * FROM Table12";
        ...
        public void call_Price_For_txtbox_Paid()
        {
            ...
            try
            {
                SqlCommand cmdTopla = new SqlCommand("SELECT price FROM Table12", baglanti);
