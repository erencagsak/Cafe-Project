...
{
    public partial class manager_Tables_Table1 : DevExpress.XtraEditors.XtraForm
    {
        BaglantiSinif bgl = new BaglantiSinif();

        // SQL bağlantılarını açar.
        public manager_Tables_Table1()
        {
            InitializeComponent();
        }
        private void manager_Tables_Table1_Load(object sender, EventArgs e)
        {
            screenSettings();
            // Ekranı ayarla.
        ...
        private void btn_Add_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (dataGriedView_Menu.SelectedRows.Count > 0)
                {
                    ...
                    using (SqlCommand ekle = new SqlCommand(@"insert into Table1 (name, number, price) values (@name, @number, @price)", baglanti))
                    {
        ...
        private void btn_Delete_Click(object sender, EventArgs e)
        {
            ...
            string sqlTable = "SELECT * FROM Table1 WHERE id=@id";
            ...
            try
            {
                if (readerTable.Read())
                {
                    ...
                    if (DialogResult.Yes == situation)
                    {
                        ...
                        string tableDeleteSQL = "DELETE FROM Table1 WHERE id=@id";
            ...
            try
            {
                ...
                if (DialogResult.Yes == situation)
                {
                    if (dataGriedView_Order.Rows.Count > 0)
                    {
                        ...
                        SqlCommand cmdTotalPrice = new SqlCommand("INSERT INTO day_TotalPrice_Tables (table_Name,table_Price) VALUES ('Masa 1','" + txtBox_Paid.Text + "')", baglanti);

                        cmdTotalPrice.ExecuteNonQuery();

                        string truncateTable = "TRUNCATE TABLE Table1";
        ...
        public void change_Appearance_data_GriedView_Order()
        {
            ...
            SqlDataAdapter adapterOrder = new SqlDataAdapter("SELECT id,name,number,price FROM Table1", baglanti);
        ...
        public void call_Price_For_txtbox_Paid()
        {
            ...
            try
            {
                SqlCommand cmdTopla = new SqlCommand("SELECT price FROM Table1", baglanti);
