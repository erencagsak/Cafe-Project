...
{
    public partial class manager_Sale : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void manager_Sale_Load(object sender, EventArgs e)
        {
            ...
            change_dataGriedView_MonthPrice();

            // dataGriedView_MonthPrice kısmına sqlden çektiği verileri yazdır. Fontu ayarla ve kolon isimlerini düzenle.

            change_txtBox_TotalMonthPrice();

            // txtBox_TotalMonthPrice textboxına sqldeki verileri toplayıp yazdır.
        }
        ...
        public void change_dataGriedView_MonthPrice()
        {
            SqlConnection baglanti = new SqlConnection(bgl.Adress);
            ...
            SqlDataAdapter adapterMontPrice = new SqlDataAdapter("SELECT id,date,price FROM month_TotalPrice_Tables", baglanti);
            DataTable tableMonthPrice = new DataTable();
            adapterMontPrice.Fill(tableMonthPrice);

            tableMonthPrice.Columns["id"].ColumnName = "ID";
            tableMonthPrice.Columns["date"].ColumnName = "Tarih";
            tableMonthPrice.Columns["price"].ColumnName = "Günlük Toplam Para";

            // Kolon isimlerini düzenle.

            dataGriedView_MonthPrice.DataSource = tableMonthPrice;

            dataGriedView_MonthPrice.DefaultCellStyle.Font = new Font("Segoe UI Semibold", 11, FontStyle.Bold);

            // Tablonun font ayarını düzenle.

            dataGriedView_MonthPrice.ColumnHeadersDefaultCellStyle.BackColor = Color.FromArgb(245,245,255);
            dataGriedView_MonthPrice.EnableHeadersVisualStyles = false;
        ...
        public void change_txtBox_TotalMonthPrice()
        {
            SqlConnection baglanti = new SqlConnection(bgl.Adress);
            ...
            try
            {
                SqlCommand cmdSum = new SqlCommand("SELECT price FROM month_TotalPrice_Tables", baglanti);
                ...
                txtBox_MonthPrice.Text = sayiTopla.ToString();
                cmdSum.Dispose();
                reader.Close();
            }
            catch (Exception)
        ...
        private void btn_ResetTotalDayPrice_Click(object sender, EventArgs e)
        {
            ...
            if (txtBox_Date.MaskFull)
            {
                ...
                if (dataGriedView_DayPrice.Rows.Count > 0)
                {
                ...
                {
                    MessageBox.Show("Günlük Ciro Kısmı Boş. Lütfen kontrol ediniz.");
                }
            }
            else
            {
